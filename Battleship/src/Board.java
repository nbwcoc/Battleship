...
public class Board extends AbstractBoard {
    ...
    public void placeShips(Ship[] shipArray) {
        ...
        for (int x = 0; x < shipArray.length; x++) {
             ...
             * direction will be used for ship placement. 0 = North, 1 = East, 2
             * = South, 3 = West
             */
            ...
            /*
             * cycle through a loop that is the size of the ship we will place
             * the ship in its initial position then we will find what direction
             * we are going with the ship, and increment or decrement the
             * coordinates until the ship length has been reached
             */
            if (checkIfValid(shipArray[x], xCoord, yCoord, direction)
                && checkBounds(shipArray[x], xCoord, yCoord, direction)) {
                for (int y = 0; y < shipArray[x].getLength(); y++) {
                    // if this specific coordinate on the board does not exist
                    // in
                    // the hashmap and it is within the bounds, then we will
                    // place the ship
                ...
                // validation check did not succeed, so we need to lower the x
                // increment to make up for the ship not being placed
                x--;
            }
        }
    }

    // this will make sure that our ship placement will not go out of the bounds
    // of the board
    ...
    private boolean checkIfValid(Ship ship, int xCoord, int yCoord,
        int direction) {
        var xy = new Coords(xCoord, yCoord);
    ...
    public void displayBoard() {
        displayImpl((Coords c) -> {
            return getMarker(hitLayer, c);
        }, (Coords c) -> {
            return getMarker(shipLayer, c);
        });
    }
}
